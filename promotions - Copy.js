...
jQuery("span[id*=_eligiblePromotions_line]").each(function(){
	...
	//alert("Inside eligiblePromotions_line ");
	...
	if(promoString.indexOf(".!.")>-1){
		...
		for(i=0; i<promoArray.length-1;i++)
		{
			...
			if(promoValues[0]==selectPromo)
			{
				...
				if(promoValues[0] != "ADVANTAGECAPPEDPRICE" && promoValues[0] != "FSFREECBC")
				{
				...
				else
				{
					if(selectPromo=="")
					{
						var lineDiscountField = "input[name*='_"+index[1]+"_override_line']";
						var promoDiscount="input[name*='_"+index[1]+"_promoDiscount_line']";
						...
						jQuery(lineDiscountField).val(promoValues[1]);
						jQuery(promoDiscount).val(promoValues[1]);							
